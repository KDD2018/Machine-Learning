...
class Processing():
    ...
    def preprocess(self, data):
        ...
        if data.loc[0, 'car_class'] == 'EV':
            del data['cylinder_number'], data['level']
    ...
    def feature_encode(self, data, col1, col2):
        ...
        df.loc[:, 'register_time'] = pd.cut(df.register_time, bins=[-1, 0.5, 1, 2, 3, 4, 5, 10, 50],
                                            labels=['半年以内', '1年以内', '1-2年', '2-3年', '3-4年', '4-5年', '5-10年','10年以上'])
        # 过户次数
    ...
    def get_category(self, car_class, car_system, car_level, classes):
        '''
        获取分类类别
        ...
        '''
        # 分类型特征名称
        cols_name_categ_NEV = ['car_system', 'level', 'car_class', 'displacement', 'driving',
                               'maximum_power', 'register_time', 'sell_times', 'model_year'] # 'intake_form','gearbox_type',
        cols_name_categ_EV = ['car_system', 'driving', 'gearbox_type', 'maximum_power', 'register_time',
        ...
        register_time = ['半年以内', '1年以内', '1-2年', '2-3年', '3-4年', '4-5年', '5-10年', '10年以上']
        ...
        categories_NEV = [displacement, driving,  maximum_power, register_time,
                          sell_times, model_year] # gearbox_type,
        categories_EV = [driving,  maximum_power, register_time, sell_times, model_year, voyage_range] # gearbox_type,
        ...
        if car_class == 'EV':  # 纯电动类型车辆的分类型特征和类别
            ...
            categories_NEV.insert(2, classes)
    ...
    def normalize(self, data):
        '''
        正则化
        :param data: 待标准化的特征数据
        :return: 标准化的数据
        '''
        scaler = StandardScaler()
        data_scaled = scaler.fit_transform(data[['vendor_guide_price', 'price']])
        df_scaled = pd.DataFrame(data_scaled, columns=['vendor_guide_price', 'price'])
        # print(scaler.mean_)
        return df_scaled, scaler.mean_, scaler.scale_
