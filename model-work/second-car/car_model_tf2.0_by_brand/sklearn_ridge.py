...
def get_brands_and_systems(sql_to_customer_carConfig, sql_to_class, sql_to_system, sql_to_level):
    '''
    获取车辆品牌集和车系集
...
def get_car_case(car_brand, sql_to_CarConfig_CarCase, car_class, classes, systems, levels):
    '''
    获取车辆案例信息
    ...
    car_case = select.get_df(sql_to_CarConfig_CarCase.format(car_brand))
    # 将同类车辆案例信息写入DataFrame
    car_case_df = pd.DataFrame(car_case, columns=car_case[0].keys())
    # print(car_case_df.head(1))
    ...
    print('\n开始离散化...')
    df_disrete = process.feature_encode(df_preprocessed, col_NEV, col_EV)  # 离散化
    df_numeric, scale_mean_, scale_std = process.normalize(df_preprocessed)

    scale_mean_ = pd.DataFrame(scale_mean_)
    scale_std = pd.DataFrame(scale_std)
    scale_mean_.to_csv('../../model-param/scale_mean_.csv', encoding='utf-8', index=False)
    scale_std.to_csv('../../model-param/scale_std.csv', encoding='utf-8', index=False)
    categories, col_categ = process.get_category(car_class, systems, levels, classes)
    # print(categories)

    print('\n开始one-hot编码...')
    df_categ = process.onehot_encode(df_disrete[col_categ], categories)  # one-hot编码
    df = pd.concat([df_categ, df_preprocessed['meter_mile'], df_numeric], axis=1)
    train_data, test_data = process.split_train_test(df, test_ratio=0.25)
    return train_data, test_data


def ridge_model(train_data, test_data, car_brand):
    '''
    训练Ridge模型
    ...
    abe =  abs(error).sum()/ len(y_test)
    print('\n**************平均绝对误差为：%f**************'%abe)

    job.dump(reg, f'../../model-param/{car_brand}.joblib')


def run():
    '''主程序'''
    car_class, car_brand, classes, systems, levels = get_brands_and_systems(sql_to_customer_carConfig, sql_to_class,
                                                                            sql_to_system, sql_to_level)
    train_data, test_data = get_car_case(car_brand, sql_to_CarConfig_CarCase, car_class, classes, systems, levels)
    ridge_model(train_data, test_data, car_brand)
...
# 非纯电动特征名称
col_NEV = ['car_system', 'level', 'car_class', 'displacement', 'driving',
           'maximum_power', 'register_time', 'meter_mile', 'sell_times', 'vendor_guide_price',
           'model_year', 'price'] #  'intake_form', 'gearbox_type',
# 纯电动特征名称
