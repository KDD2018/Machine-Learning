...
class GenerateCSV():
    ...
    def get_car_case(self, sql_to_customer_carConfig, sql_to_brand, sql_to_system, sql_to_CarConfig_CarCase):
        '''
        获取车辆案例信息
    ...
    def write2csv(self, df, batch_size, car_class):
        '''
        将数据框分批次写入多个csv
        :param batch_size: 每批次写入CSV的样本数量
        :param car_class: 车辆类型
        '''
        # train_df, test_df = split_train_(df, 0.2)
        epochs = math.ceil(df.shape[0] / batch_size)
        print('\n**********************开始写入CSV文件*****************************')
    ...
    def run(self):
        ...
        car_class, car_case_df, brands, systems = self.get_car_case(sql_to_customer_carConfig, sql_to_brand, sql_to_system,
        ...
        # 3、将处理后的数据写入CSV文件
        self.write2csv(df=df, batch_size=50000, car_class=car_class)

        return len(df.columns), car_class
