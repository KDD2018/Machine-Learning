...
class ConvBNReLU(layers.Layer):
    def __init__(self, out_channel, kernel_size=3, strides=1, **kwargs):
        ...
        self.conv = layers.Conv2D(filters=out_channel, kernel_size=kernel_size, strides=strides,
...
class InvertedResidual(layers.Layer):
    def __init__(self, in_channel, out_channel, strides, expand_ratio, **kwargs):
        ...
        layer_list.extend([
            # 3x3 depthwise conv
            layers.DepthwiseConv2D(kernel_size=3, strides=strides, padding='same', use_bias=False, name='depthwise'),
            layers.BatchNormalization(momentum=0.9, epsilon=1e-5, name='depthwise/BatchNorm'),
            layers.ReLU(max_value=6.0),
            # 1x1 pointwise conv(linear)
            layers.Conv2D(filters=out_channel, kernel_size=1, strides=1, padding='same',
...
def MobileNetV2(img_height=224, img_width=224, num_classes=1000, alpha=1.0, round_nearest=8):
    ...
    for t, c, n, s in inverted_residual_setting:
        ...
        for i in range(n):
            strides = s if i ==2 else 1
            x = block(x.shape[-1], output_channel, strides, expand_ratio=t)(x)
    # last several layers
