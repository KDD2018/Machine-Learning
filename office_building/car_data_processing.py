...
def preprocess(data):
    ...
    data['meter_mile'] = data.loc[:, 'meter_mile'] / 10000.0  # 换算成万公里
    data['car_price'] = data.loc[:, 'car_price'] / 10000.0  # 换算成万元
    data = data[data.meter_mile < 50]  # 过滤掉50万公里以上的案例
    # data = data[data.car_price > 1]  # 过滤掉车价低于１万的
    ...
    data['car_price'] = data['car_price'].map(lambda x: math.log(x))
...
def feature_encode(data):
    ...
    data['annual_inspect'] = pd.cut(data.annual_inspect, bins=[-20, -10, -5, 0, 5],
                                          labels=['10年以前', '5年以前', '过去5年', '未来5年'])
    return data
...
def model_select(model_name, X_train, X_test, X_valid, y_train, y_test, y_valid, my_car_info):
    ...
    your_car_price = math.exp(log_price)
...
def my_car():
    ...
    car_info = {'brand_name': brand_name, 'car_system': car_system, 'car_model_name': car_model_name,
                ...
                'compulsory_insurance': compulsory_insurance, 'model_year': model_year, 'car_level': car_level,
                'car_price': 0.1}
...
col = ['brand_name', 'car_system', 'car_model_name', 'register_time', 'meter_mile', 'gearbox', 'displacement',
       'sell_times', 'annual_inspect', 'compulsory_insurance', 'model_year', 'car_level', 'car_price']
